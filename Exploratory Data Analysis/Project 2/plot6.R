...
plot6 <- function () {
  ...
  Total_PM25_By_Year <- filter(NEI, (fips %in% c("24510", "06037") ) & (SCC  %in% motor_vehicle_related_SCC) ) %>%
                      select(year, fips, Emissions) %>%  
                      group_by(year, fips)  %>% 
                      summarise(tot_Emision = sum(Emissions) )
 # Change the fips to factors 
 Total_PM25_By_Year$fips <- sapply(Total_PM25_By_Year$fips, factor)
 # Change the levels of fips
 levels(Total_PM25_By_Year$fips) <- c("Los Angeles County", "Baltimore City")
